\chapter{Integrating NoSQL in the Virtual Observatory} % (fold)
...
\item the MongoDB-provided Java driver\urlnote{http://docs.mongodb.org/ecosystem/drivers/java/}; or
...
% NOTA: No se explica qué hace esto; ¿por qué la cláusula where("pid")?

\begin{lstlisting}[float,label=lst:classic_new,caption=Old vs new syntax Mongo-Java connection, retrieving all columns from a table containing the columns description.]
...
\paragraph*{Defining a MongoDB schema for ObsCore} % (fold)
...
\paragraph*{Ingesting data in the ObsCore database} % (fold)
...
% (and their particular metadata to standard models that in some cases need to be completed with metadata not available in FITS headers) files to the VO.
...
% paragraph ingesting_data_in_the_obscore_database (end)

\paragraph*{Adapting the existing Java code} % (fold)
...
We can see that the classes to be changed within the OpenCADC are a small subset. Notably, the classes dealing with the TAP endpoints, parsing of TAP queries, job management (UWS pattern), etcetera, remain untouched, and only the classes most directly involved with database interfacing need to be changed. That is partly thanks to the architecture of the OpenCADC, where classes are assembled through dependency injection, and responsibilities are clearly delimitated by the interfaces.

% paragraph adapting_the_existing_java_code (end)

% chapter integrating_nosql_in_the_virtual_observatory (end)
