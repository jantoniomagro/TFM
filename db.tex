...
After studying NoSQL DBMS and showing their pros, in this section we assume we want to rewrite the Web application from ASA shown in \ref{fig:asa_implementation} to implement MongoDB features.

To take advantage of the existing OpenCADC work, our proposal is to use MongoDB and Java programming language, with Spring framework\urlnote{http://www.springsource.org}, a comprehensive programming and configuration model for Java-based enterprise applications. The Spring Data MongoDB \urlnote{http://www.springsource.org/spring-data/mongodb} provides an extension to the Spring programming model that support writing applications for the new datastores, like MongoDB or CouchDB database. There is no need to use any additional component, provided we have this software. \newline
...
% Let's see some examples extracted from ALMA query interface code in \ref{lst:tap_schema_dao_java}, and then, its equivalent using the interface between Spring Data MongoDB and MongoDB DBMS in \ref{lst:tap_schema_dao_mongo}: 
%  
% \begin{lstlisting}[float,label=lst:tap_schema_dao_java,caption=ASA existing code to select and list all rows from \emph{TAP\_SCHEMA.colums} table.]
%  
% // select all rows from TAP_SCHEMA.colums.
...
%  \end{lstlisting}
% 
% \begin{lstlisting}[float,label=lst:tap_schema_dao_mongo,caption=Spring Data MongoDB code to select and list all rows from \emph{TAP\_SCHEMA.colums} table.]
%  
% private static final String FIND_COLUMNS = "asa_columns ";
% 
% public List<Columns> findAllColumns() {
%   mongoTemplate.find(FIND_COLUMNS), new Query(), Columns.class);
% }
% 
%  \end{lstlisting}